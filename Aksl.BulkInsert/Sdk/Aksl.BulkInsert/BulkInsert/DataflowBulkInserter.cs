...
namespace Aksl.BulkInsert
{
    ...
    public class DataflowBulkInserter<TMessage, TResult> : IDataflowBulkInserter<TMessage, TResult>
    {
        ...
        protected void InitializeBulkInserter(Func<IEnumerable<TMessage>, Task<IEnumerable<TResult>>> insertHandler, BlockSettings blockSettings, ILoggerFactory loggerFactory)
        {
            ...
            _logger.LogInformation($"{nameof(DataflowBulkInserter<TMessage, TResult>)}'s Initialize");
        }
        #endregion
                        ...
                        try
                        {
                            using (await _mutexResult.LockAsync())
                            {
                                ...
                                if ((resuls?.Any()).HasValue)
                                {
                                    ...
                                    _logger
                                       .LogInformation($"{nameof(CreateBlockers)}'s ExecutionTime={sw.Elapsed},Count=\"{resuls?.Count()}\",count/time(sec):{Math.Ceiling((long)resuls?.Count() / sw.Elapsed.TotalSeconds)},ThreadId={Thread.CurrentThread.ManagedThreadId},now:\"{DateTime.Now.TimeOfDay}\"");
                                }
                            }
                        }
                        catch (Exception ex)
